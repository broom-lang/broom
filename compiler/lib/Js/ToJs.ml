...
let emit program =
    ...
    let emit_label (label : Cont.Id.t) name =
    ...
    let rec emit_transfer parent (transfer : Transfer.t) =
        ...
        let rec emit_expr id =
            ...
            | None ->
                ...
                let expr = match term with (* OPTIMIZE: unbox tuples (in an earlier pass) *)
                    | Values vals -> surround_separate_map 4 0 (brackets empty)
                    ...
                    | Label label ->
                        ...
                        let name_doc = emit_label label cont.name in
        ...
        let transfer = match transfer.term with
            | Jump {callee; universals = _; args} ->
                parens (emit_expr callee) (* OPTIMIZE: don't always need parens *)
        ...
        ^^ string "return" ^^ blank 1 ^^ transfer ^^ semi
